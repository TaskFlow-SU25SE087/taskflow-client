...
export function DroppableBoard({ boardId, children, className = '' }: DroppableBoardProps) {
  ...
  return (
    ...
    >
      {children}
      {isOver && isEmpty && (
        <div className={'absolute inset-0 flex items-center justify-center pointer-events-none select-none text-violet-500 font-semibold text-base animate-pulse bg-violet-50/80 rounded-2xl'}>
          Thả task vào đây
        </div>
      )}
    </div>
