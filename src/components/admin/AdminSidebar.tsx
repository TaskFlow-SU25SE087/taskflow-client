    ...
    <>
      ...
      >
        <div className='px-4 sm:px-6 mb-6 sm:mb-8 flex items-center justify-between'>
          <h2
            className={`text-lg sm:text-xl font-bold text-gray-800 transition-opacity ${
        ...
        <nav className='flex-1 flex flex-col gap-1 sm:gap-2 px-2'>
          ...
          <NavLink
            ...
            onClick={onClose}
            title={isCollapsed ? 'Import Users' : undefined}
          >
            <Users className='h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0' />
            <span className={`transition-opacity ${isCollapsed ? 'opacity-0 hidden' : 'opacity-100'}`}>
              Import Users
        ...
        {/* Footer section */}
        <div
          className={`px-4 sm:px-6 mt-auto pt-4 border-t border-gray-200 transition-opacity ${
            isCollapsed ? 'opacity-0 hidden' : 'opacity-100'
          }`}
        >
          <div className='text-xs text-gray-500 text-center'>Admin Panel By Taskflow</div>
        </div>
      </div>
    </>
  )
