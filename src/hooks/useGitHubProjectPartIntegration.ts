...
export function useGitHubProjectPartIntegration() {
  ...
  const checkConnectionStatus = useCallback(async (projectId: string, partId: string) => {
    setLoading(true);
    try {
  ...
  const startGitHubOAuth = useCallback(async () => {
    ...
    try {
      ...
      if (response.code === 200) { // Sửa điều kiện ở đây
  ...
  const createNewProjectPart = useCallback(async (projectId: string, partData: {
    name: string;
    programmingLanguage: string;
    framework: string;
  }) => {
  ...
  const connectRepoToPart = useCallback(async (projectId: string, partId: string, repoUrl: string, accessToken: string) => {
