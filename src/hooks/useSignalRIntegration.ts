...
export const useSignalRIntegration = () => {
  ...
  const joinProjectGroup = useCallback(async (projectId: string) => {
    if (!isConnected) {
  ...
  const leaveProjectGroup = useCallback(async (projectId: string) => {
    if (!isConnected) {
  ...
  const listenForTaskUpdates = useCallback((taskId: string, callback: (notification: NotificationData) => void) => {
  ...
  const listenForProjectUpdates = useCallback((projectId: string, callback: (notification: NotificationData) => void) => {
    const handleNotification = (notification: NotificationData) => {
